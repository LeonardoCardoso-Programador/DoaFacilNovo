...
import com.example.doafacilnovo.databinding.ActivityVizualizarSolicitacaoBinding
...
class VizualizarSolicitacaoActivity : AppCompatActivity() {

    private val binding by lazy { ActivityVizualizarSolicitacaoBinding.inflate(layoutInflater) }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.buttonAceitar.setOnClickListener{
            acaoBotaoAdicionar()
        }
    ...
    private fun fetchSolicitacoes(solicitacaoUid: String) {
        db.collection("solicitacaoDoacao").document(solicitacaoUid)
            ...
            .addOnSuccessListener { document ->
                ...
                fetchUserBeneficiario(uidBeneficiario)
    ...
    private fun fetchUserBeneficiario(uidBeneficiario: String) {
        db.collection("users").document(uidBeneficiario) // Obtém um único documento pelo UID
            .get()
            .addOnSuccessListener { result ->
                ...
                if (result.exists()) {
                    ...
                    if (!currentImageName.isNullOrEmpty()) {
                        loadImageFromFirebaseStorage(currentImageName!!, uidBeneficiario)
                    }

                    // Atualizar os campos de texto, verificando se os valores são não nulos
    ...
    private fun acaoBotaoAdicionar(){

        // Atualizar dados no Firestore
        val solicitacapData: MutableMap<String, Any> = hashMapOf(
            "estadoSolicitacao" to "Solicitação aceita"

        )
            // Se não há imagem, apenas atualizamos os dados no Firestore
            db.collection("solicitacaoDoacao").document(solicitacaoUid)
                .update(solicitacapData)
        ...
        // Buscar o número de telefone do beneficiário
        db.collection("users").document(uidBeneficiario).get()
            .addOnSuccessListener { userDoc ->
                val telefoneBeneficiario = userDoc.getString("telefone").orEmpty()

                if (telefoneBeneficiario.isNotEmpty()) {
                    // Construir a mensagem a ser enviada
                    val mensagem = "Olá, eu aceitei sua solicitação!!! Estou pronto para seguir com a doação."

                    // Intent para abrir o WhatsApp com a mensagem
                    val intent = Intent(Intent.ACTION_VIEW).apply {
                        data = Uri.parse("https://wa.me/$telefoneBeneficiario?text=$mensagem")
                    }
                    ...
                    Toast.makeText(this, "Telefone do beneficiário não encontrado.", Toast.LENGTH_SHORT).show()
                }
            }
            .addOnFailureListener {
                Toast.makeText(this, "Erro ao buscar telefone do beneficiário.", Toast.LENGTH_SHORT).show()
            }

    }


    private fun loadImageFromFirebaseStorage(imageName: String, uidBeneficiario: String) {
        val storageRef = FirebaseStorage.getInstance().reference.child("fotosUsuarios/$uidBeneficiario/$imageName")
