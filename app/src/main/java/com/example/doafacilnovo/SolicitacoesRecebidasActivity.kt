...
class SolicitacoesRecebidasActivity : AppCompatActivity() {
    private val binding by lazy { ActivitySolicitacoesRecebidasBinding.inflate(layoutInflater) }
    ...
    private fun fetchSolicitacoes() {

        db.collection("solicitacaoDoacao").whereEqualTo("userUidDoador",userUid)
            .get()
                        ...
                        .addOnSuccessListener { doacaoDoc ->
                            ...
                            db.collection("users").document(uidBeneficiario).get()
                                .addOnSuccessListener { userDoc ->
                                    ...
                                    val solicitacao = Solicitacoes(nomeBeneficiario, "Solicitou " + tituloDoacao, uidSolicitacao)
    ...
    private fun updateRecyclerView(solicitacoes: List<Solicitacoes>) {
        val adapter = ListaSolicitacao(this, solicitacoes) { solicitacao -> // Correção do nome da variável
            val intent = Intent(this, VizualizarSolicitacaoActivity::class.java).apply {
